...
<%
    ...
    if (object not_eq NULL and CFGetTypeID(object) == CFNumberGetTypeID()) <%
        long number;
...
<%
    ...
    if (not usagePageRef or not usageRef) <%
        quit();
    %>
    ...
    if (result not_eq kIOReturnSuccess) <%
        quit();
    %>
    ...
    for (CFIndex i = 0; i < CFArrayGetCount(elements); i++) <%
        ...
        if (
            get_long_value(element, CFSTR(kIOHIDElementUsagePageKey)) == kHIDPage_LEDs and
